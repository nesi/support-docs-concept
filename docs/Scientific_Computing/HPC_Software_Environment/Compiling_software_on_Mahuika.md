...
compilers:
  ...
  --------------- -------------------- ---------------------------------- ---------------
  Debugging       `-g` or              `-g or -g{0,1,2,3}`                Set level of
  ...
  Light compiler  `-O2`                `-O2`                               
  optimisation                                                            

  Aggressive      `-O3 -ipo`           `-O3 -ffast-math -funroll-loops`   This may affect
  compiler                                                                numerical
  optimisation                                                            accuracy

  Architecture    `-xHost`             `-march=native -mtune=native`      Build and
  specific                                                                compute nodes
  optimisation                                                            have the same
                                                                          architecture
                                                                          (Broadwell)

  Vectorisation   `-qopt-report`       `-fopt-info-vec` or                 
